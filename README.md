...
The extension makes no external network requests. It does not use the Gmail API, Google APIs, OAuth, analytics, or telemetry, and it does not store email content.

Send Guard for Gmailは、Gmail Web版の送信前に件名・宛先ドメイン・添付ファイル・CC/BCC・確認チェックを確認する、ローカル処理のChrome拡張です。

この拡張機能は外部通信を行いません。Gmail API、Google API、OAuth、アクセス解析、テレメトリを使用せず、メール本文等も保存しません。設定は`chrome.storage.local`にのみ保存します。
...
5. Select the extracted repository's `extension/` directory.
6. Reload Gmail Web and test a compose window.
...
## Manual QA and Gmail Compatibility

Basic manual verification on real Gmail Web has been completed by the maintainer for the initial public release.

Automated tests and basic manual verification do not replace the full real-browser checklist. Gmail DOM is not a stable public API, so reply/forward flows, pop-out compose windows, Auto CC/BCC behavior, keyboard shortcuts, Schedule Send, attachment upload states, multiple compose windows, language switching, performance, and future Gmail changes require continued verification.
...
- Repository: [shogot62/send-guard-for-gmail](https://github.com/shogot62/send-guard-for-gmail)
- Author: shogot62
